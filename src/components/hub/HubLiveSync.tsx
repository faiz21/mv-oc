...
const LIVE_TABLES = ['tasks', 'approval_queue', 'audit_log', 'agents', 'system_state']

export function HubLiveSync() {
  ...
  useEffect(() => {
    ...
    const channels = LIVE_TABLES.map((table) =>
      supabase
        .channel(`hub:${table}`)
        .on(
          'postgres_changes',
